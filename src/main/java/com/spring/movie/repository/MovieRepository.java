...
@Mapper
public interface MoiveRepository {

    // 사전에 정의해야 하는 메서드들
    // 테이블 생성
    void createMoiveTable();// 이 메서드 호출시 CREATE TABLE 구문 실행

    // 더미데이터 입력
    void insertTestData();// 더미데이터로 3개 row 데이터 입력하기

    // 테이블 삭제
    void dropMoiveTable();// 단위테스트 종료 후 DB초기화를 위해 테이블 삭제
